...
pub struct Config {
    ...
    pub buckets: HashMap<u32, BucketConfig>,
...
impl Config {
    pub fn from_yaml_str(s: &str) -> Result<Self, ConfigError> {
        ...
        for bucket in raw_config.buckets.0 {
            ...
            buckets.insert(bucket.from_chain_id, bucket);
        }
...
impl ValidateUniqueItems for TokenConfigs {
    fn validate_unique_items(&self) -> Result<(), UniqueItemsError> {
        self.0.iter().map(|t| t.symbol.clone()).collect::<Vec<_>>().validate_unique_items()
    }
}
...
pub struct TokenConfig {
    ...
    pub by_chain: HashMap<u32, ChainSpecificTokenConfig>,
}
